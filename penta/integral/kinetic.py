...
def kinetic(alpha,beta,A,B,a,b):
###########################################################################################################
#Initial Conditions
        s=np.zeros((3,7,7))
	ki=np.zeros((3,7,7))
...
#Initial Conditions
	for i in range(3):
		P[i]=(alpha*A[i]+beta*B[i])/(alpha+beta)
		ki[i][0][0]=2*alpha*beta*s[i][1][1]
#Recurrence Index
		for j in range(6):
			if j>0:
				ki[i][j][0]=-j*beta*s[i][j-1][1]+2*alpha*beta*s[i][j+1][1]
		for j in range(6):
			if j>0:
				ki[i][0][j]=-j*alpha*s[i][1][j-1]+2*alpha*beta*s[i][1][j+1]
#Transfer Equation
		...
		for k in range(6):
			if k>0:	
				for j in range(6):
					if j>0:
						ki[i][j][k]=j*k*s[i][j-1][k-1]-2*j*beta*s[i][j-1][k+1]-2*k*alpha*s[i][j+1][k-1]+4*alpha*beta*s[i][j+1][k+1]/2.0
	EAB=math.exp(-alpha*beta/(alpha+beta)*np.dot((A-B),(A-B)))
	return EAB*(math.pi/(alpha+beta))**1.5*(ki[0][a[0]][b[0]]*s[1][a[1]][b[1]]*s[2][a[2]][b[2]]+s[0][a[0]][b[0]]*ki[1][a[1]][b[1]]*s[2][a[2]][b[2]]+s[0][a[0]][b[0]]*s[1][a[1]][b[1]]*ki[2][a[2]][b[2]])
...
def Kinetic_Matrix(OrbCoeff,FCenter,CartAng):
        ...
        for p in range(len(OrbCoeff)):
                for q in range(len(OrbCoeff)):
                        ...
                        for i in range(3):
                                for j in range(3):
                                        Sum=Sum+NormCoeff(OrbCoeff[p][i],CartAng[p])*NormCoeff(OrbCoeff[q][j],CartAng[q])*PrimCoeff[p][i]*PrimCoeff[q][j]*kinetic(OrbCoeff[p][i],OrbCoeff[q][j],FCenter[p],FCenter[q],CartAng[p],CartAng[q])
                        M[p][q]=Sum
        return M
